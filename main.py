    ...
    if message.text == 'Часовой пояc':
        ...
        while info == "":
            ...
            for i in lessons.keys():
                if next_today != today or int(i.split(":")[0]) > int(timing.split(":")[0]) or (int(i.split(":")[0]) == int(timing.split(":")[0]) and int(i.split(":")[1]) > int(timing.split(":")[1])):
                    info = f'Следующий урок в {["пн", "вт", "ср", "чт", "пт", "сб", "вс"][next_today - 1]} в {i}\n{lessons[i]}'
                    break
